  ...
  const fetch = async () => {
    ...
    const channelPosts =
      channelTagCode !== undefined
        ? await tagPosts(channelCode, channelTagCode) // 태그가 있으면 태그별 게시글 가져오기
        : await allPosts(channelCode); // 태그가 없으면 모든 게시글 가져오기
    ...
    <>
      <div className="main-box">
        ...
        <div className="tag-box">
          <Link className="tag" to={"/channel/" + channelCode}>
            전체
          </Link>
          {channel?.channelTag.map((channelTag) => (
            <Link
              className="tag"
              to={"/channel/" + channelCode + "/" + channelTag.channelTagCode}
